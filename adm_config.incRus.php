...
define('SERVER_START_FILE', 'start_server.exe'); // этот файл нужен для запуска сервер через админ панель,
//при заходе через меню в опции Конт.Сервера -> Контроль, админ панель должна автоматом скопировать файлы start_server.exe и start_server.bat
// которые находится в папке installation в папку с игрой
//  при старте сервера с админ панели запустится 
//  файл  start_server.exe который запуст файл start_server.bat  это позволит правильно отобразить статус сервера после его запуска
