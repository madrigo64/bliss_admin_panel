...
//  файл  start_server.exe который запустистя от локального юзера в системе что позволит запустить HiveExt Debug Monitor без которого сервер работать правильно не будет
//   start_server.bat  который нужно предворительно отредактировать, он в свою очередь запустит сервер
// если вы запускаете Apache как простую программу позаботьтесь о том, что бы файл апача httpd.exe запускался с админ правами
